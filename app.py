...
# Function for Applying Darker Filter to image
def dark(img):
    img_dark = cv2.convertScaleAbs(img, beta=-50)
    display(img_dark, "Darker Image")


# Function for converting image to Gray image
def greyscale(img):
...
# Function for converting image to HDR image
def HDR(img):
...
# Function for converting image to Warm image
def Winter(img):
    ...
    display(summ, "Warm Image")


# Function for converting image to Cold image
def Summer(img):
    increaseLookupTable = LookupTable([0, 64, 128, 256], [0, 80, 160, 256])
    decreaseLookupTable = LookupTable([0, 64, 128, 256], [0, 50, 100, 256])
    blue_channel, green_channel, red_channel = cv2.split(img)
...
def cartoonify(Original):
    ...
    display(NoiseFree, "Noise free Image")
